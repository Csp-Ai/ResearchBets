...
13. The BEFORE-stage slip builder now reuses that exact reviewed summary boundary via `src/core/slips/preSubmitPatternWarning.ts`: it computes an advisory-only `PreSubmitPatternWarning` contract (`warning_level`, `matched_patterns`, `recommendation_summary`, `sample_size`, `confidence_level`, optional `suppression_reason`) against the current draft slip and renders only when deterministic history plus current-slip signals support it.
