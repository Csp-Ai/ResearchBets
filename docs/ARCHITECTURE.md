...
16. The shared ticket-thesis layer (`src/core/slips/ticketThesis.ts`) composes the already-canonical lifecycle risk, action guidance, evidence, and supported continuity metadata into one deterministic narrative object (`headline`, `current_thesis`, `primary_pressure`, `recommended_next_step`, `why_now`, continuity read, constrained thesis status). BEFORE, DURING, and AFTER surfaces consume that object instead of improvising separate advisory copy, so the bettor sees one narrative spine across submit, live tracking, and review.
...
- Screenshot parsing is adapter-driven inside `src/core/bettor-memory/parser.ts`. The parser layer first builds a minimal OCR/text extraction contract, then performs conservative sportsbook classification, adapter selection, normalized candidate extraction, and parser provenance/warning persistence. Raw uploads stay authoritative, parser output stays editable, and verified review data keeps precedence in analytics/postmortems.
...
- Credibility/coverage calculations live in `src/core/bettor-memory/credibility.ts` and extend the existing bettor-memory foundation. The layer computes deterministic counts/percentages for verified, parser-derived, demo/fallback, review-needed, and parse-failed records without introducing a parallel trust subsystem.
