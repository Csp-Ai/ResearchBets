...
export function savePostmortem(record: PostmortemRecord) {
  ...
  const deduped = [normalized, ...existing.filter((item) => item.ticketId !== normalized.ticketId)].slice(0, 100);
...
export function getDraftPostmortem(ticketId: string): DraftPostmortemSnapshot | undefined {
  return readJson<DraftPostmortemSnapshot[]>(DRAFT_KEY, []).map(migrateDraft).find((item) => item.ticketId === ticketId);
}
