...
const HALF_LIFE_BY_TYPE: Record<InsightType, number> = {
  ...
  model_snapshot: 60,
  delta_snapshot: 40,
  outcome_snapshot: 720,
  edge_realized: 720,
  calibration_update: 360
};

export function createInsightId(input: {
  gameId: string;
  agentKey: string;
  ...
  const digest = createHash('sha256')
    .update(`${input.gameId}:${input.agentKey}:${input.insightType}:${bucket}`)
    .digest('hex')
    .slice(0, 16);
...
export function buildInsightNode(input: {
  ...
  modelImplied?: number;
  attribution?: InsightNode['attribution'];
}): InsightNode {
  const timestamp = input.timestamp ?? new Date().toISOString();
  const delta =
    typeof input.modelImplied === 'number' && typeof input.marketImplied === 'number'
      ? Number((input.modelImplied - input.marketImplied).toFixed(4))
      : undefined;
  return {
    insight_id: createInsightId({
      gameId: input.gameId,
    ...
    timestamp,
    decay_half_life: HALF_LIFE_BY_TYPE[input.insightType],
...
export function summarizeInsightGraph(nodes: InsightNode[]): InsightGraphSummary {
  ...
  } satisfies Record<InsightType, number>;
  for (const node of nodes) countsByType[node.insight_type] += 1;

  const fragilityVariables = [...nodes]
    .sort(
      (a, b) =>
        (1 - b.confidence) * (Math.abs(b.delta ?? 0.2) + 0.25) -
        (1 - a.confidence) * (Math.abs(a.delta ?? 0.2) + 0.25)
    )
    .slice(0, 3);

  const disagreementProxyByType = Object.entries(countsByType).reduce(
    (acc, [key]) => {
      const typedKey = key as InsightType;
      const confidences = nodes
        .filter((node) => node.insight_type === typedKey)
