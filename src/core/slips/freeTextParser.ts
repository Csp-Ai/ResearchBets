...
const MARKET_PATTERNS: Array<{ market: MarketType; pattern: RegExp }> = [
  ...
  {
    market: 'anytime_td',
    pattern: /\b(?:any\s*time|anytime)\s+(?:touchdown|td)(?:\s+scorer)?\b|\battd\b/i
  },
...
const cleanSelection = (line: string, market: MarketType | null): string => {
  ...
  cleaned = cleaned
    .replace(
      /\b(?:over|under|alt|odds|nfl|nba|mlb|nhl|ufc|football|basketball|baseball|hockey|soccer)\b/gi,
      ' '
    )
    .replace(/\b[ou]\s*(?=\d)/gi, ' ')
    .replace(/\b\d+(?:\.\d+)?\s*\+/g, ' ')
...
export const parseSlipText = (rawText: string): { legs: ParsedSlipLeg[]; confidence: number } => {
  ...
  const matchedSport =
    Object.entries(SPORT_KEYWORDS).find(([key]) => normalizedBlock.includes(key))?.[1] ?? null;
  ...
  const parsedLegs = lines
    .map((line) => {
      ...
      return {
        ...
        marketType: market,
        line: market === 'anytime_td' ? 1 : toLine(lineMatch?.[1]),
