...
import type {
  ...
  ScriptCluster,
  SlipStructureReport
} from '@/src/core/contracts/slipStructureReport';
...
const normalizeMatchupLabel = (value?: string): string | undefined => {
  const normalized = value?.trim().replace(/\s+/g, ' ');
  return normalized ? normalized : undefined;
};

const buildDerivedMatchupKey = (
  leg: Pick<SlipIntelLeg, 'matchup' | 'home' | 'away'>
): string | undefined => {
  const matchup = normalizeMatchupLabel(leg.matchup);
  if (matchup) return `matchup:${matchup}`;

  const home = normalizeMatchupLabel(leg.home);
  const away = normalizeMatchupLabel(leg.away);
  if (home && away) return `matchup:${away} @ ${home}`;
  return undefined;
};

export const inferGameKey = (
  leg: Pick<SlipIntelLeg, 'game' | 'game_id' | 'event_id' | 'matchup' | 'home' | 'away'>
): string => {
  const explicitGameId = normalizeMatchupLabel(leg.game_id ?? leg.game);
  if (explicitGameId) return explicitGameId;

  const eventId = normalizeMatchupLabel(leg.event_id);
  if (eventId) return `event:${eventId}`;

  const derivedMatchup = buildDerivedMatchupKey(leg);
  if (derivedMatchup) return derivedMatchup;

  return 'game:unlabeled';
};

export const inferPlayerKey = (leg: Pick<SlipIntelLeg, 'player' | 'selection'>): string => {
  const player = normalizeMatchupLabel(leg.player);
  if (player) return `player:${player}`;

  const selection = normalizeMatchupLabel(leg.selection);
...
const toSlipLeg = (leg: SlipIntelLeg, index: number): SlipLeg => ({
  ...
  odds: leg.odds,
  notes: leg.selection,
  side: normalizeLabel(leg.selection).includes(' under ')
    ? 'under'
    : normalizeLabel(leg.selection).includes(' over ')
      ? 'over'
      : undefined
});
...
const bandFromScore = (score: number): 'low' | 'med' | 'high' =>
  score >= 67 ? 'high' : score >= 40 ? 'med' : 'low';
...
export function buildSlipStructureReport(
  ...
  const draftLegs = Array.isArray(input)
    ? (input as Array<SlipIntelLeg | SlipLeg>).map((leg, index) =>
        'leg_id' in leg ? (leg as SlipLeg) : toSlipLeg(leg as SlipIntelLeg, index)
      )
    : input.legs;
  ...
  for (const leg of legs) {
    ...
    if (
      lineLooksAlt(typeof leg.line === 'number' ? `${leg.line}` : undefined) ||
      (odds !== null && odds >= 120)
    )
      altLineLegs += 1;
  }
  ...
  const legRanked = legs
    .map((leg) => {
      const odds = parseAmericanOdds(typeof leg.odds === 'number' ? `${leg.odds}` : leg.odds);
      const isAggressiveLine = lineLooksAlt(
      ...
      if ((playerCounts.get(inferPlayerKey(toIntelLeg(leg))) ?? 0) > 1) {
        flags.push('same_player_dependency');
        localFragility += 18;
      }
      if ((gameCounts.get(inferGameKey(toIntelLeg(leg))) ?? 0) > 1) {
  ...
  for (let i = 0; i < legs.length; i += 1) {
    for (let j = i + 1; j < legs.length; j += 1) {
      const a = legs[i]!;
      const b = legs[j]!;
      if (inferPlayerKey(toIntelLeg(a)) === inferPlayerKey(toIntelLeg(b))) {
        correlation_edges.push({
          a_leg_id: a.leg_id,
          b_leg_id: b.leg_id,
          kind: 'same_player',
          severity: 'high',
          reason: 'Multiple outcomes rely on the same player.'
        });
        ...
        correlation_edges.push({
          ...
          kind: 'same_team',
          severity: 'med',
          reason: 'Multiple legs are tied to the same team result.'
        });
      } else if (inferGameKey(toIntelLeg(a)) === inferGameKey(toIntelLeg(b))) {
        correlation_edges.push({
          a_leg_id: a.leg_id,
          b_leg_id: b.leg_id,
          kind: 'same_game',
          severity: 'low',
          reason: 'Shared game script can affect both legs.'
        });
      }
    }
  }
  ...
  const script_clusters: ScriptCluster[] = sameGameGroups.map(([game, count], index) => ({
    ...
    leg_ids: legs.filter((leg) => inferGameKey(toIntelLeg(leg)) === game).map((leg) => leg.leg_id),
  ...
  if (longshotLegs > 0)
    reasons.push(
      `${longshotLegs} longshot leg${longshotLegs === 1 ? '' : 's'} increase tail risk.`
    );
  if (altLineLegs > 0)
    reasons.push(
      `${altLineLegs} leg${altLineLegs === 1 ? '' : 's'} use aggressive/alt-style lines.`
    );
  if (reasons.length === 0)
    reasons.push('No major concentration flags detected; risk is spread across legs.');
...
export function computeSlipIntelligence(legs: SlipIntelLeg[]): SlipIntelligence {
  ...
  if (report.legs.length === 0) {
    return {
      ...
      weakestLegHints:
        report.failure_forecast.top_reasons.length > 0
          ? report.failure_forecast.top_reasons
          : ['No legs yet — add props to see structure risk.']
    };
  }
  ...
  return {
    ...
    weakestLegHints:
      report.failure_forecast.top_reasons.length > 0
        ? report.failure_forecast.top_reasons
        : ['No major concentration flags detected; risk is spread across legs.']
  };
}
