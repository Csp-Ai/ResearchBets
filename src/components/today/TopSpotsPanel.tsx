...
export function TopSpotsPanel({ scouts, onSelect, selectedLegIds }: { scouts: TodayPropKey[]; onSelect?: (id: string) => void; selectedLegIds?: Set<string> }) {
  ...
  return (
    <section className="space-y-2" data-testid="top-spots-panel">
      ...
      <div className="grid gap-2 sm:grid-cols-2">
        {scouts.map((scout) => (
          <button key={scout.id} type="button" onClick={() => onSelect?.(scout.id)} className={`rounded-xl border p-3 text-left ${selectedLegIds?.has(scout.id) ? 'border-emerald-300/45 bg-emerald-500/10' : 'border-white/10 bg-slate-950/40 hover:border-cyan-400/40'}`}>
            ...
            <p className="mt-1 text-xs text-slate-400">{selectedLegIds?.has(scout.id) ? 'Staged in ticket · ' : ''}L10 {scout.hitRateL10 ?? 0}% · {(scout.riskTag ?? 'watch') === 'stable' ? 'Steadier setup' : 'Higher swing setup'}</p>
