    ...
    <>
      {!compact ? (
        <div className={`${styles.snapshotToast} ${effectiveMode === 'live' ? styles.live : ''}`}>
          ...
          {loading ? 'Loading telemetry snapshot…' : `${label} · ${snapshot?.gamesCount ?? 0} games tracked`}
        </div>
      ) : null}
      <div className={styles.snapshotCard}>
        ...
        <div className={styles.snapshotBody}>
          <div className={styles.snapshotAsOf}>As of {snapshot?.lastUpdatedAt ? new Date(snapshot.lastUpdatedAt).toTimeString().slice(0, 8) : '—'}</div>
          <div className={styles.providerChips}>
            {loading ? (
              <span className={styles.skeleton} style={{ width: 160, height: 22 }} />
            ) : (
              <span className={`${styles.providerChip} ${effectiveMode === 'live' ? styles.ok : styles.warn}`}>
                {effectiveMode === 'live' ? '✅' : '⚠️'} {reasonText[snapshot?.reason ?? 'unknown'] ?? reasonText.unknown}
              </span>
            )}
          </div>
          ...
          <div className={styles.snapshotRow}>
            <span className={styles.oddsLabel}>Mode reason</span>
            <span className={styles.oddsValue}>{reasonText[snapshot?.reason ?? 'unknown'] ?? reasonText.unknown}</span>
          </div>
        </div>
