...
export function RiskGauge() {
  ...
  return <section className={styles.gaugeSection}><div className={styles.gaugeInner}><div className={styles.gaugeCopy}><div className={styles.sectionLabel}>Animated risk. Phase 02.</div><h2>Your risk.<br />Live. Leg<br />by leg.</h2><p>Hover each leg to see how it&apos;s affecting total slip risk. The gauge animates in real time as risk shifts — not a static number, a live read.</p><a href="/ingest" className={styles.btnPrimary}>Stress test your slip →</a></div><div className={styles.gaugeWidget}><div className={styles.gaugeTitle}>Slip risk breakdown</div><div className={styles.gaugeArcWrap}><canvas ref={ref} /><div className={styles.gaugePct}>{pct}%</div></div><div className={styles.gaugeLegs}>{GAUGE_LEGS.map((leg) => <div key={leg.name} className={styles.gaugeLegRow} onMouseEnter={() => { setPct(leg.risk); setTip(`<span class="hl">${leg.name}</span> — ${leg.tip}`); }} onMouseLeave={() => { setPct(62); setTip('Hover a leg for AI context.'); }}><span className={styles.gaugeLegDot} style={{ background: leg.color }} /><span className={styles.gaugeLegName}>{leg.name}</span><span className={styles.gaugeLegBarWrap}><span className={styles.gaugeLegBar} style={{ width: `${leg.risk}%`, background: leg.color }} /></span><span className={styles.gaugeLegRisk}>{leg.risk}%</span></div>)}</div><div className={styles.gaugeTooltip} dangerouslySetInnerHTML={{ __html: tip }} /></div></div></section>;
}
