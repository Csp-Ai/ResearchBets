...
export function RiskGauge() {
  ...
  return <section className={styles.gaugeSection}><div className={styles.gaugeInner}><div className={styles.gaugeCopy}><div className={styles.sectionLabel}>Risk intelligence</div><h2>Your slip risk,<br />leg by leg.</h2><p className={styles.sectionCaption}>Identify which leg contributes most downside before you submit.</p><a href="/ingest" className={styles.btnPrimary}>Analyze my slip</a></div><div className={styles.gaugeWidget}><div className={styles.gaugeTitle}>Slip risk breakdown</div><div className={styles.gaugeArcWrap}><canvas ref={ref} /><div className={styles.gaugePct}>{pct}%</div></div><div className={styles.gaugeLegs}>{GAUGE_LEGS.map((leg) => <div key={leg.name} className={styles.gaugeLegRow} onMouseEnter={() => { setPct(leg.risk); setTip(`<span class="hl">${leg.name}</span> — ${leg.tip}`); }} onMouseLeave={() => { setPct(62); setTip('Hover a leg for AI context.'); }}><span className={styles.gaugeLegDot} style={{ background: leg.color }} /><span className={styles.gaugeLegName}>{leg.name}</span><span className={styles.gaugeLegBarWrap}><span className={styles.gaugeLegBar} style={{ width: `${leg.risk}%`, background: leg.color }} /></span><span className={styles.gaugeLegRisk}>{leg.risk}%</span></div>)}</div><div className={styles.gaugeTooltip} dangerouslySetInnerHTML={{ __html: tip }} /></div></div></section>;
}
