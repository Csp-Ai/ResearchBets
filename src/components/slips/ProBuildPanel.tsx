...
export function ProBuildPanel({ legs, onApply }: { legs: SlipBuilderLeg[]; onApply: (nextLegs: SlipBuilderLeg[]) => void }) {
  ...
  return (
    <details className="rounded-xl border border-white/[0.07] bg-white/[0.015] p-3" data-testid="pro-build-panel">
      <summary className="cursor-pointer list-none text-xs font-semibold text-slate-300">
        Advanced construction math <span className="font-normal text-slate-500">· optional</span>
      </summary>
      <div className="mt-3 space-y-3">
        <div className="flex items-center justify-between">
          <p className="text-xs text-slate-400">Deterministic construction diagnostics</p>
          <Badge size="sm" variant="info">Advanced</Badge>
        </div>
        {activeGuardrail ? (
          <div className="w-fit rounded-full border border-cyan-400/40 bg-cyan-400/10 px-2 py-1 text-[11px] text-cyan-100">
            Guardrail: {activeGuardrail.title}
          </div>
        ) : null}
        ...
        <div className="terminal-divider rounded-lg bg-slate-950/50 p-3 text-xs text-slate-300">
          <p className="mono-number text-sm">Proxy {(independentProb * 100).toFixed(1)}% <span className="text-slate-500">|</span> Break-even {(breakEven * 100).toFixed(1)}% <span className="text-slate-500">|</span> <span className={gapTone}>Gap {(probabilityGap * 100).toFixed(1)}%</span></p>
          <p className="mt-1 text-slate-500">Deterministic proxy only; not the primary ResearchBets ticket answer.</p>
        </div>
        <div className="space-y-1">
          {constraints.warnings.map((warning) => <p key={warning} className="text-xs text-amber-200">• {warning}</p>)}
        </div>
        <div className="grid grid-cols-2 gap-2">
          <Button intent="secondary" title={legs.length < 2 ? 'Need at least 2 legs' : `Preview keeps: ${previewSelection(2) || 'n/a'}`} className="min-h-0 py-2 text-xs" onClick={() => applyProSize(2)} disabled={legs.length < 2}>Apply 2-leg</Button>
          <Button intent="primary" title={legs.length < 3 ? 'Need at least 3 legs' : `Preview keeps: ${previewSelection(3) || 'n/a'}`} className="min-h-0 py-2 text-xs" onClick={() => applyProSize(3)} disabled={legs.length < 3}>Apply 3-leg</Button>
        </div>
      </div>
    </details>
  );
