...
function needsReview(leg: TrackedTicketLeg) {
  return (
    leg.parseConfidence === 'low' || !leg.player.trim() || !leg.marketType || !(leg.threshold > 0)
  );
}
...
function asTrackedMode(value: unknown): TrackedTicket['mode'] {
  return value === 'live' || value === 'cache' || value === 'demo' ? value : 'demo';
}
...
export function TrackSlipInput({ onTracked, onOpenDraft, onTrySample, sampleLoading }: Props) {
  ...
  const updateLeg = (legId: string, patch: Partial<TrackedTicketLeg>) => {
    setParsedTicket((current) => {
      ...
      return {
        ...current,
        legs: current.legs
          .map((leg) => (leg.legId === legId ? { ...leg, ...patch } : leg))
          .map((leg) => ({ ...leg, needsReview: needsReview(leg) }))
      };
    });
  };
  ...
  const confirmTracked = () => {
    ...
    const ticketId =
      parsedTicket.ticketId.startsWith('ticket_') && parsedTicket.ticketId.length > 20
        ? parsedTicket.ticketId
        : `ticket_${crypto.randomUUID()}`;
    const normalizedTicket: TrackedTicket = {
      ...parsedTicket,
      trace_id: parsedTicket.trace_id ?? nervous.trace_id,
      run_id: parsedTicket.run_id ?? parsedTicket.trace_id ?? nervous.trace_id,
      slip_id: parsedTicket.slip_id ?? nervous.slip_id,
      sport: parsedTicket.sport ?? nervous.sport,
      tz: parsedTicket.tz ?? nervous.tz,
      date: parsedTicket.date ?? nervous.date,
      mode: asTrackedMode(parsedTicket.mode ?? nervous.mode),
      ...
      cashoutValue:
        typeof parsedTicket.cashoutValue === 'number' ? parsedTicket.cashoutValue : undefined,
      legs: parsedTicket.legs.map((leg, index) => ({
        ...leg,
        legId: leg.legId || `leg-${index + 1}`,
        needsReview: needsReview(leg)
      }))
    };
    ...
    return (
      <section className="mt-4 rounded-lg border border-slate-700 bg-slate-950/40 p-3">
        ...
        <div className="mt-2 flex flex-wrap gap-2 text-[11px]">
          {getLoopTrustBadges(parsedTicket.provenance).map((badge) => (
            <span
              key={badge.label}
          ...
          <table className="w-full min-w-[760px] text-xs">
            <thead className="text-left text-slate-300">
              <tr>
                ...
                <th className="pb-2">Over/Under</th>
            ...
            <tbody>
              {parsedTicket.legs.map((leg, index) => (
                <tr key={leg.legId} className="border-t border-slate-800">
                  ...
                  <td className="py-2 pr-2">
                    <input
                      aria-label={`market-${index + 1}`}
                      className="w-full rounded border border-slate-700 bg-slate-950 p-1"
                      value={leg.marketType}
                      onChange={(event) =>
                        updateLeg(leg.legId, {
                          marketType: (event.target.value ||
                            'points') as TrackedTicketLeg['marketType']
                        })
                      }
                    />
                  </td>
                  <td className="py-2 pr-2">
                    <input
                      ...
                      type="number"
                      step="0.1"
                      className="w-20 rounded border border-slate-700 bg-slate-950 p-1"
                      value={Number.isFinite(leg.threshold) ? leg.threshold : 0}
                      onChange={(event) =>
                        updateLeg(leg.legId, { threshold: Number(event.target.value) })
                      }
                    />
                  </td>
                  <td className="py-2 pr-2">
                    <select
                      ...
                      value={leg.direction}
                      onChange={(event) =>
                        updateLeg(leg.legId, { direction: event.target.value as 'over' | 'under' })
                      }
                    >
                  ...
                  <td className="py-2 pr-2">
                    <input
                      aria-label={`odds-${index + 1}`}
                      className="w-20 rounded border border-slate-700 bg-slate-950 p-1"
                      value={leg.odds ?? ''}
                      onChange={(event) => updateLeg(leg.legId, { odds: event.target.value })}
                    />
                  </td>
                  <td className="py-2 pr-2">
                    ...
                    {needsReview(leg) ? (
                      <span className="ml-1 rounded-full border border-amber-300/40 bg-amber-500/10 px-2 py-0.5">
                        Needs review
                      </span>
                    ) : null}
                  </td>
                      ...
                      onClick={() =>
                        setParsedTicket((current) =>
                          current
                            ? {
                                ...current,
                                legs: current.legs.filter((item) => item.legId !== leg.legId)
                              }
                            : current
                        )
                      }
              ...
              setParsedTicket((current) =>
                current
                  ? {
                      ...
                      legs: [
                        ...current.legs,
                        createLeg(current.legs.length + 1, current.sourceHint)
                      ]
                    }
                  : current
              )
        ...
        <div className="mt-3 flex flex-wrap gap-2 text-sm">
          <button
            type="button"
            className="rounded-lg border border-cyan-400/60 bg-cyan-500/10 px-3 py-2 text-cyan-100"
  ...
  return (
    <section className="mt-4 rounded-lg border border-slate-700 bg-slate-950/40 p-3">
      <label className="text-xs text-slate-300" htmlFor="track-slip-text">
        Paste slip
      </label>
      <textarea
        id="track-slip-text"
      ...
      <div className="mt-3 flex flex-wrap gap-2 text-sm">
        <button
          type="button"
          className="rounded-lg border border-cyan-400/60 bg-cyan-500/10 px-3 py-2 text-cyan-100 disabled:opacity-60"
          onClick={() => void trackText(rawText, 'paste')}
        ...
        <button
          type="button"
          className="rounded-lg border border-emerald-400/60 bg-emerald-500/10 px-3 py-2 text-emerald-100 disabled:opacity-60"
        ...
        <Link
          href={appendQuery(nervous.toHref('/tonight'), {})}
          className="rounded-lg border border-white/20 px-3 py-2"
        >
          Open Tonight
        </Link>
      ...
      <p className="mt-2 text-[11px] text-slate-400">
        Tracked routes preserve trace continuity:{' '}
        {withTraceId(nervous.toHref('/track'), nervous.trace_id ?? 'trace_demo_track')}
      </p>
    </section>
  );
}
