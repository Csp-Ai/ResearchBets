...
vi.mock('@/src/core/events/useRunEvents', () => ({
  useRunEvents: () => ({
    events: [{ type: 'stage_analyze_started', ts: new Date().toISOString() }],
    latestStage: 'analyzing',
    statusText: 'Analyzing slip'
  })
}));
...
describe('cockpit run status', () => {
  ...
  it('reflects analyzing stage text from realtime events hook in ticket rail', async () => {
    ...
    const ticketPanel = screen
      .getByRole('heading', { level: 2, name: 'Draft Ticket' })
      .closest('section');
