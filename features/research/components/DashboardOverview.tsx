...
export function DashboardOverview({ bets }: DashboardOverviewProps) {
  ...
  return (
    <section className="rounded-xl border border-slate-800 bg-slate-900 p-6">
      <h2 className="text-xl font-semibold">Research Loop Snapshot</h2>
      <p className="mt-1 text-sm text-slate-400">Bet Slip → Confirm → Track → Alert → Insight</p>
      <dl className="mt-4 grid gap-3 sm:grid-cols-3">
        <div className="rounded-lg border border-slate-800 bg-slate-950 p-4">
          <dt className="text-sm text-slate-400">Open Bets</dt>
          <dd className="text-2xl font-semibold">{openCount}</dd>
        </div>
        <div className="rounded-lg border border-slate-800 bg-slate-950 p-4">
          <dt className="text-sm text-slate-400">Settled Bets</dt>
