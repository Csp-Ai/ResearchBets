...
export function LandingHero() {
  ...
  return (
    <motion.section
      ...
      initial="hidden"
      animate="visible"
      className="relative isolate flex min-h-[100svh] items-center overflow-hidden px-4 py-8 sm:px-6 lg:px-10"
    >
      <HeroDepthLayer />
      ...
      <motion.div
        ...
        initial="hidden"
        animate="visible"
        className="relative z-20 flex w-full items-start justify-center pt-20 sm:pt-24 lg:pt-28"
      >
        <motion.div
          variants={staggerItem}
          className={`terminal-panel terminal-panel-shell relative w-[78vw] max-w-[1300px] rounded-xl p-5 sm:p-6 lg:p-8 ${
              ...
              return (
                <motion.div
                  ...
                  className={`rounded-md border px-3 py-2 text-xs ${
                    isWeak ? 'border-amber-400/55 bg-amber-500/10 text-amber-100' : 'border-slate-700 bg-slate-900/80 text-slate-200'
                  }`}
                >
        ...
        <div className="pointer-events-none absolute left-0 top-0 z-30 w-full max-w-[560px] px-2 sm:px-0">
          <motion.p variants={staggerItem} className="text-xs uppercase tracking-[0.22em] text-cyan-300/90">
            Research Terminal
          </motion.p>
          <motion.h1 variants={staggerItem} className="mt-2 text-3xl font-semibold leading-tight sm:text-4xl lg:text-5xl">
            Run the slip. Isolate fragility. Decide with precision.
          </motion.h1>
          <motion.p variants={staggerItem} className="mt-4 max-w-xl text-sm text-slate-300">
            ResearchBets surfaces weak links, confidence trajectory, and risk posture in one tactical stream. No picks, no
            hype, just operational betting analysis.
          </motion.p>
          <motion.div variants={staggerItem} className="pointer-events-auto mt-6 flex flex-wrap gap-3">
            ...
            <Link
              ...
              className="interactive-button rounded-md border border-slate-600 bg-slate-900/70 px-4 py-2 text-sm"
            >
              Launch Demo Verdict
            </Link>
          </motion.div>
